...
\noindent
Supplement to the manuscript \emph{Workflow-transfer benchmark of optical surface-roughness measurements against an internal tactile baseline across engineering materials and manufacturing processes}.
...
\medskip
\noindent
The main manuscript contains the decision-oriented benchmark tables, fixed-workflow sensitivity analysis, $R_{sm}$ diagnostic and unit-scale sensitivity analyses, and leave-one-surface workflow-transfer check. The supplement extends those results with the archive-level ANOVA screens, wavelength regressions, descriptive summaries, tactile repeatability summaries, bootstrap stability intervals, and representative figures. Group-level CSV outputs for the main-manuscript decision, fixed-workflow, and workflow-transfer analyses are included in the public reproducibility package.
...
\input{supplement_tables.tex}

\clearpage
\section{Bootstrap stability of aggregate medians}
\noindent
The table below complements the main manuscript decision summaries by adding percentile bootstrap intervals around the aggregate retrospective lowest-discrepancy medians by parameter, process, and material. Resampling was performed within each aggregate stratum across archive-defined parameter--surface groups.

\input{tables/results_bootstrap_ci_aggregate_medians.tex}
